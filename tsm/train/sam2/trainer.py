...
from tsm.train.sam2.optimizer import construct_optimizer

from tsm.train.sam2.utils.checkpoint_utils import (
    assert_skipped_parameters_are_frozen,
    exclude_params_matching_unix_pattern,
    load_state_dict_into_model,
    with_check_parameter_frozen,
)
from tsm.train.sam2.utils.data_utils import BatchedVideoDatapoint
from tsm.train.sam2.utils.distributed import all_reduce_max, barrier, get_rank

from tsm.train.sam2.utils.logger import Logger, setup_logging

from tsm.train.sam2.utils.train_utils import (
    AverageMeter,
    collect_dict_keys,
    DurationMeter,
    get_amp_type,
...
class Trainer:
    ...
    def _setup_ddp_distributed_training(self, distributed_conf, accelerator):
        ...
        if distributed_conf.comms_dtype is not None:  # noqa
            ...
            if amp_type == torch.bfloat16:
                ...
                logger.info("Enabling bfloat16 grad communication")
            else:
                hook = ddp_comm_hooks.default_hooks.fp16_compress_hook
                logger.info("Enabling fp16 grad communication")
            process_group = None
        ...
        if self.mode == "train":
            if self.epoch > 0:
                ...
                if self.is_intermediate_val_epoch(self.epoch - 1):
                    logger.info("Running previous val epoch")
                    self.epoch -= 1
                    self.run_val()
                    self.epoch += 1
            self.run_train()
    ...
    def train_epoch(self, train_loader):
        ...
        for k, v in extra_loss_mts.items():
            out_dict[k] = v.avg
        out_dict.update(self._get_trainer_state(phase))
        logger.info(f"Losses and meters: {out_dict}")
    ...
    def _log_meters_and_save_best_ckpts(self, phases: List[str]):
        logger.info("Synchronizing meters")
        out_dict = {}
    ...
    def _log_timers(self, phase):
        ...
        logger.info(f"Estimated time remaining: {human_readable_time(time_remaining)}")
    ...
    def _setup_components(self):
        ...
        logger.info("Setting up components: Model, loss, optim, meters etc.")
        self.epoch = 0
        ...
        logger.info("Finished setting up components: Model, loss, optim, meters etc.")
...
def print_model_summary(model: torch.nn.Module, log_dir: str = ""):
    ...
    logger.info("==" * 10)
    logger.info(f"Summary for model {type(model)}")
    logger.info(f"Model is {model}")
    logger.info(f"\tTotal parameters {get_human_readable_count(total_parameters)}")
    logger.info(
